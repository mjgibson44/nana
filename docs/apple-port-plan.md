...
| — | **v9/v10: Occupy's board** — the appendix in [`apple-port-notes/protocol.md`](apple-port-notes/protocol.md) is the record; in one line, v9 made the Occupy board unbounded (`size` → `frame`) and added `zones`, and v10 made every word its own player's (`notYours` / `throughRival`, value read off same-owner runs) and turned a zone into a minute-long contest that pays `OccupyState.bonuses` at the whistle. Both ride the existing `state` snapshot, so the bump is only there to keep boards of different rules out of the same random match. |
